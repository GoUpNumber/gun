            ...
            for id in ids {
                match bet_db.get_entity::<BetState>(id) {
                    Ok(Some(bet_state)) => match bet_state {
                        ...
                        BetState::Won { .. } | BetState::Claimed { height: None, .. } | BetState::Cancelled { height: None, .. } | BetState::Confirmed { .. }  => return Err(anyhow!("You may not forget bet {} because it is in the {} state", id, bet_state.name())),
                        _ => to_remove.push(id),
                    },
